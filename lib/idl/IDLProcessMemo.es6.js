...
class IDLProcessMemo extends Memo {
  ...
  loadIDL({idlPaths, refPath}) {
    ...
    let idlMap = {};
    const allIDL = idls.reduce((acc, idl) => acc.concat(idl), []);
    allIDL.forEach(idl => idlMap[this.hashObject(idl)] = idl);
    let idl = [];
    Object.keys(idlMap).forEach(key => idl.push(idlMap[key]));
  ...
  hashObject(o) {
    ...
    if (oNoFiles.files) delete oNoFiles.files;
  ...
  groupParses({idl}) {
    ...
    data.grouped = _.groupBy(idl, parse => {
      ...
      // TODO(markdittmer): This is hack to deal with failure to
      // differentiate partial and non-partial Dictionaries and Namespaces
      // with their own constructor.
      let typeName = parse.constructor.name;
      if (parse.isPartial && typeName.indexOf('Partial') !== 0)
  ...
  gatherSuper(deduped, newParse, parses, superName) {
    if (parses.length > 0) {
      ...
      for (const member of gatheredInherited.members) {
        // Sometimes diamond inheritance or similar leads to pulling in
        // identical members multiple times. Guard against that.
        const memberNoFrom = this.withoutFrom(member);
        if (newParse.members.filter(
          m => _.isEqual(this.withoutFrom(m), memberNoFrom)
        ).length > 0) {
          this.logger.warn(`${newParse.name || newParse.implementer}: Skipping mixed-in member from ${superName}: Identical member already found`);
          continue;
        }
  ...
  withoutFrom(parse) {
    let newParse = deepClone(parse);
    if (newParse.from) delete newParse.from;
    return newParse;
  }
