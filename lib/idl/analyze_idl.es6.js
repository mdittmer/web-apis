...
e('#copy-report').addEventListener('click', () => {
  window.getSelection().selectAllChildren(e('#output'));
...
function analyzeAllIDLs(logger) {
  const names = _.uniq(data.left.map(left => left.name || left.implementer));
  for (const name of names) {
    analyzeIDL(logger, name);
  }
}

function analyzeIDL(logger, name) {
  logger.log(`Analyzing ${name}`);
  ...
  if (!left) {
    ...
    logger.log(`Loaded left ${name} from ${left.url}`);
  }
  ...
  if (!right) {
    ...
    logger.log(`Loaded right ${name} from ${right.url}`);
  }
