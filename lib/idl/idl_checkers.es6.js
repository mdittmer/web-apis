...
Right: ${stringify(ra)})`);
  ...
  function leftAddsNonStandardAPIs(logger, left, right) {
    if (!left.members) return;
    const name = left.name || left.implementer;
    if (!right.members) {
      logger.warn(`${name}: Right has no members`);
    ...
    for (const lm of lms) {
      ...
      const rmsf = rms.filter(rm => rm.name === memberName);
      if (rmsf.length === 0) {
        if (nonstdNames && nonstdNames.length > 0) { nonstdNames = nonstdNames.concat(", ", memberName); }
        else { nonstdNames = memberName; }
      }
    } 
    if (nonstdNames && nonstdNames.length > 0) {
       logger.error(`${name}: Left adds non-standard members:\n    ${nonstdNames}`);
    }
  },
];
