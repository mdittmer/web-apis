...
module.exports = [
  ...
  function rightMissingConstructor(logger, left, right) {
    ...
    function hasConstructor(idl) {
      return idl.attrs && idl.attrs.some(isCtor);
    }
    const chk = check.bind(this, logger);
    if (hasConstructor(left) && !hasConstructor(right)) {
      const ctors = JSON.stringify(left.attrs.filter(isCtor), null, 2);
      logger.error(
        `Parse "${right.name || right.implementer}" is missing Constructor.
        Right has constructors: ${ctors}`
      );
    }
  },
];
