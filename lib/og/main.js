...
var ObjectGraph = og.BrowserObjectGraph;
...
document.body.querySelector('#collect').addEventListener('click', function() {
  ...
  var graph = new ObjectGraph({
    ...
    onDone: function() {
      ...
      graph.environment = {
        browser: {
          name: browserNameE.value,
          version: browserVersionE.value,
        },
        platform: {
          name: platformNameE.value,
          version: platformVersionE.value,
        },
      };
      graph.userAgent = navigator.userAgent;
