...
document.body.querySelector('#collect').addEventListener('click', function() {
  ...
  var graph = new ObjectGraph({
    ...
    onDone: function() {
      ...
      graph.environment.browser.name =
          body.querySelector('#browser-name').value;
      graph.environment.browser.version =
          body.querySelector('#browser-version').value;
      graph.environment.platform.name =
          body.querySelector('#platform-name').value;
      graph.environment.platform.version =
          body.querySelector('#platform-version').value;
