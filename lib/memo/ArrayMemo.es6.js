...
class ArrayMemo extends Memo {
  ...
  collectValues(o, delegateValues) {
    // Unpack arrays in the no-delegates and single-delegate cases.
    if (delegateValues.every(valuesArray => valuesArray.length <= 1)) {
      return super.collectValues(
        o, delegateValues.reduce((acc, arr) => acc.concat(arr), [])
      );
    }

    return super.collectValues(o, delegateValues);
  }
}
