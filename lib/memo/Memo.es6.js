...
class Memo extends Base {
  ...
  runAllDispatch_(output) {
    return this.dispatchAllToDelegates(output).then(
      this.collectValues_.bind(this, {output})
    );
  }

  runAllThrow_(error) {
    return this.throwAllToDelegates(error).then(
      this.collectValues_.bind(this, {error})
    );
  }

  collectValues_(o, delegateValues) {
    ...
    if (this.keep || o.error) {
      if (keepValues.length > 0) o.delegates = keepValues;
      return o;
    }
    console.log(`keepValues: ${JSON.stringify(keepValues)}`);
